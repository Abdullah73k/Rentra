...
const LeaseCard = ({ lease }: { lease: NewPropertyBuildType["lease"] }) => {
  return (
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="font-semibold text-foreground">
            {lease?.start && new Date(lease.start).toLocaleDateString()}
          </p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">End Date</p>
          <p className="font-semibold text-foreground">
            {lease?.end && new Date(lease.end).toLocaleDateString()}
          </p>
        </div>
      </div>
      <div>
