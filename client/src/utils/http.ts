...
export async function sendAiQuery(message: string) {
	try {
		const res = await axios.post(
		...
		return res.data.data as { content: string; threadId: string };
	} catch (error) {
		throw new Error("An error occurred while querying the AI assistant");
	}
}
